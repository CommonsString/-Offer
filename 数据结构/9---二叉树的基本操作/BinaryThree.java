...
public class BinaryThree {
	 ...
	 * 根节点, 左右子树, 取其最大值
	 */
	public static int recurGetMaxValue(BinaryThree head){
		if(head == null) return 0;
		int headVal = head.getData();
		int leftValue = getMaxValue(head.getLeft());
		int rightValue = getMaxValue(head.getRight());
		int max = Integer.MIN_VALUE;
		max = leftValue > rightValue ? leftValue : rightValue;
		max = headVal > max ? headVal : max;
		return max;
	}
	 ...
	 * 	利用层次遍历, 寻找最大值
	 */
	public static int getMaxValue(BinaryThree head){
		//参数检查
		if(head == null) return 0;
		Queue<BinaryThree> myQueue = new LinkedList<>();
		myQueue.offer(head);
		int max = Integer.MIN_VALUE;
		while(!myQueue.isEmpty()){
			BinaryThree current = myQueue.poll();
			if(current.getData() > max){
				max = current.getData();
			}
			if(current.getLeft() != null){
				myQueue.offer(current.getLeft());
			}
			if(current.getRight() != null){
				myQueue.offer(current.getRight());
			}
		}
		return max;
	}	
	...
	/**
	 * 先将根节点加入辅助队列
	 * 之后在每个结点出队的时候访问数组, 同时如果出队结点左右子树不为空, 入队。
	 */
	public static void layerOrder(BinaryThree head){
		if(head == null) return ;
		Queue<BinaryThree> myQueue = new LinkedList<>();
		//根节点入队
		myQueue.offer(head);
	...
	public static void main(String[] args) {
		...
		System.out.println(getMaxValue(head));
	}
